...
namespace ConsoleApp1
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Pokemon wildPokemon2 = new Pokemon("Bruh", PokemonType.Water, PokemonType.Fire);

            // Which pokemon should Ash use?

            foreach (var pokemon in pokemonOfAsh)
            {
                if(pokemon.IsEffectiveAgainst(wildPokemon) == true)
                {
                    Console.WriteLine("I choose you, " + pokemon.Name);
                    break;
                }
            }
            ...
            foreach (var pokemon in pokemonOfAsh)
            {
                if (pokemon.IsEffectiveAgainst(wildPokemon2) == true)
                {
                    Console.WriteLine("I choose you, " + pokemon.Name);
                    //break;   >>  if you want to use the first one which matches the condition
                }
            }
